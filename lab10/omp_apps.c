...
void v_add_optimized_adjacent(double* x, double* y, double* z) {
  ...
  // int thread_sum = omp_get_num_threads(); <- This will return 1.
  // Because it is not in the parallel block.
  #pragma omp parallel
  {
    int thread_sum = omp_get_num_threads();
    int thread_id = omp_get_thread_num();
    for(int i = thread_id; i < ARRAY_SIZE; i += thread_sum)
      z[i] = x[i] + y[i];
  }
}
...
void v_add_optimized_chunks(double* x, double* y, double* z) {
  ...
  #pragma omp parallel
  {
    thread_sum = omp_get_num_threads();
    int thread_id = omp_get_thread_num();
    total_task_num = ARRAY_SIZE / thread_sum;
    for(int i = thread_id * total_task_num; i < (thread_id + 1) * total_task_num; i++)
      z[i] = x[i] + y[i];
  }
  for (int i = thread_sum * total_task_num; i < ARRAY_SIZE; i++)
    z[i] = x[i] + y[i];
}
// END PART 1 EX 2
...
double dotp_manual_optimized(double* x, double* y, int arr_size) {
  ...
  double global_sum = 0.0;
  double* z = malloc(sizeof(double) * arr_size);
